...
static bool innodb_init()
{
  ...
  if (err == DB_SUCCESS)
  {
    ...
    if (FILE *f= fopen(get_log_file_path().c_str(), "w"))
    {
      ...
      if (sizeof log_hdr_buf != fwrite(log_hdr_buf, 1, sizeof log_hdr_buf, f))
        err= DB_IO_ERROR;
      if (fclose(f))
        err= DB_IO_ERROR;
    }
  ...
  msg("mariabackup: innodb_init() returned %d (%s).", err, ut_strerr(err));
  return true;
}
...
static bool xtrabackup_copy_logfile()
{
  ...
  for (unsigned retry_count{0};;)
  {
    ...
    if (recv_sys.parse_mtr(STORE_IF_EXISTS) == recv_sys_t::OK)
    {
      ...
      if (ds_write(dst_log_file, recv_sys.buf, recv_sys.recovered_offset))
      {
        mysql_mutex_unlock(&log_sys.mutex);
        mysql_mutex_unlock(&recv_sys.mutex);
