...
@param nonce part of the initialization vector
@param src   source data
@param dst   encrypted data
@param end   pointer past the end of src */
static void log_encrypt_buf(uint64_t nonce,
                            const byte *src, byte *dst, const byte *const end)
{
  alignas(8) byte iv[MY_AES_BLOCK_SIZE];
  mach_write_to_8(iv, nonce);

  for (const byte *l= src; l != end; )
  {
    src= l;
    const byte b= *l++;
    size_t rlen= b & 0xf;
    ...
    ut_ad(l + rlen <= end);
    uint len= static_cast<uint>(rlen);
    ut_a(MY_AES_OK == encryption_crypt(l, len, dst, &len,
...
ATTRIBUTE_NOINLINE void mtr_t::encrypt()
{
  ...
  const size_t size{m_log.size()};
  ut_ad(size);

  /* We will copy all data to a contiguous buffer, because
  encryption_crypt() must be invoked with the same lengths both for
  encryption and decryption.

  FIXME: Use a smaller buffer. We need to encrypt the payload of each
  record (less than innodb_page_size bytes) separately.

  But, a single record may currently span multiple mtr_buf_t::block_t.

  If a single record was always stored contiguously, we could easily
  invoke log_encrypt_buf() on each block. */

  byte *src= static_cast<byte*>(alloca(size));
  byte *dst= static_cast<byte*>(alloca(size));

  {
    byte *s{src};
    m_log.for_each_block([this,&s](const mtr_buf_t::block_t *b)
    {
      ::memcpy(s, b->begin(), b->used());
      s+= b->used();
      return true;
    });
    ut_ad(s == src + size);
  ...
  log_encrypt_buf(m_commit_lsn, src, dst, src + size);

  m_crc= my_crc32c(0, dst, size);

  {
    const byte *d{dst};
    m_log.for_each_block([this,&d](mtr_buf_t::block_t *b)
    {
      ::memcpy(b->begin(), d, b->used());
      d+= b->used();
      return true;
    });
    ut_ad(d == dst + size);
